...
export const useLoadSession = (): (() => Promise<void>) => {
  ...
  return useCallback(async () => {
    try {
      const { user, workspaceId } = await api.fetchSession()
      set({ status: 'ready', user, workspaceId })
    } catch (err) {
      if (!isUnauthorized(err)) console.error('session load failed', err)
      set({ status: 'ready', user: null, workspaceId: null })
    }
  }, [set])
}
...
export const useSignIn = (): ((username: string, password: string) => Promise<void>) => {
  ...
  return useCallback(
    async (username, password) => {
      const { user, workspaceId } = await api.login(username, password)
      set({ status: 'ready', user, workspaceId })
    },
    [set],
  )
