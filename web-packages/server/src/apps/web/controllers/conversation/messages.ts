...
export const registerMessages: Controller = app => {
  ...
  app.post('/:cid/messages', async c => {
    ...
    if (isResponse(found)) return found
    if (!found) return notFound(c, 'conversation not found')
    if (found.workerId === null)
      return failWith(c, 409, 'worker_required', 'conversation needs a worker')
    ...
    // what decides.
    if (started) app.$commands.publish(found.workerId, { type: 'work_available' })
