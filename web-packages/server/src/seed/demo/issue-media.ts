...
if (config.isProduction || /prod/i.test(config.databaseUrl)) {
  throw new Error('refusing to seed issue media into a production database')
}
if (!config.oss) throw new Error('OSS configuration is required to seed issue media')
...
try {
  ...
  if (!context) throw new Error('run the demo and issue seeds before issue media')
  ...
  for (const item of media) {
    ...
    const issue = await app.$prisma.issue.findUnique({
      where: { appId_number: { appId: context.id, number: item.issueNumber } },
      select: { id: true, revisions: { where: { number: 1 }, select: { id: true } } },
    })
    const revision = issue?.revisions[0]
    if (!issue || !revision) throw new Error(`#${item.issueNumber} demo issue is missing`)
    await app.$prisma.issueFile.upsert({
      where: { issueId_fileId: { issueId: issue.id, fileId: file.id } },
      create: { issueId: issue.id, fileId: file.id, role: item.role, position: 0 },
      update: { role: item.role, position: 0 },
    })
    await app.$prisma.issueRevisionFile.upsert({
      where: { revisionId_fileId: { revisionId: revision.id, fileId: file.id } },
      create: { revisionId: revision.id, fileId: file.id, role: item.role, position: 0 },
      update: { role: item.role, position: 0 },
    })
    console.log(`  linked ${item.filename} to #${item.issueNumber}`)
  }
} finally {
  await dispose()
}
