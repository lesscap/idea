...
import { branchName, ensureRepo, ensureWorktree, repoLayout } from './worktree.ts'
...
describe('a repository for a key with no remote', () => {
  ...
  it('commits without a global git identity', () => {
    expect(() => ensureRepo(root, 'fresh', null)).not.toThrow()
  })
})

describe('a worktree for a conversation', () => {
  const key = 'scratch'
  const repo = () => ensureRepo(root, key, null).path

  it('is created on its own branch', () => {
    repo()
    const path = ensureWorktree(root, key, 7)
  ...
  it('keeps the session directory outside every worktree', () => {
    repo()
    const worktree = ensureWorktree(root, key, 7)
    const { sessions } = repoLayout(root, key)
  ...
  it('survives its worktree being deleted', () => {
    ...
    rmSync(ensureWorktree(root, key, 7), { recursive: true, force: true })
  ...
  it('gives separate conversations separate directories', () => {
    ...
    expect(ensureWorktree(root, key, 7)).not.toBe(ensureWorktree(root, key, 8))
  })
  ...
  it('is rebuilt from its branch after the directory is deleted', () => {
    repo()
    const first = ensureWorktree(root, key, 7)
    ...
    const rebuilt = ensureWorktree(root, key, 7)
  ...
  it('does nothing when the worktree is already there', () => {
    ...
    expect(ensureWorktree(root, key, 7)).toBe(path)
